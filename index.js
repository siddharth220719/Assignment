...
app.get("/getTimeStories", (req, res) => {
  ...
  request("https://time.com", function (error, response, body) {
    ...
    while (key != 6) {
      startofHeading =
        startofHeading === undefined
          ? str.indexOf(h2)
          : str.indexOf(h2, startofHeading);
    ...
    for (const title in headlines) {
      getLinks[title] =
        "https://time.com/" +
        headlines[title].slice(
          headlines[title].indexOf(linkstart) + 1,
          headlines[title].indexOf(linkeend)
        );
    }
    ...
    let titlestart = "/>";
    for (const title in headlines) {
      gettitles[title] = headlines[title].slice(
        headlines[title].indexOf(titlestart) + 2,
        headlines[title].length
      );
    }
    //sending final data
